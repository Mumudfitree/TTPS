<?php
    ...
    include_once('../connection.php');
                    ...
                    //query
                    $sql1 ="SELECT COUNT(login_id) from  login_information , user_role , user_data WHERE login_information.user_id = user_data.user_id AND login_information.user_role_id = user_role.user_role_id ";
                    if(isset($_GET['search'])){ 
                        $search = $_GET['search'];
                        $sql1 = "SELECT COUNT(login_id) from  login_information , user_role , user_data WHERE login_information.user_id = user_data.user_id AND login_information.user_role_id = user_role.user_role_id AND firstname LIKE '%" . $search . "%'";
                    ...
                    $sql = "SELECT * from login_information , user_role , user_data WHERE login_information.user_id = user_data.user_id AND login_information.user_role_id = user_role.user_role_id  ORDER BY login_id DESC $limit";
                    if(isset($_GET['search'])){ 
                        $search = $_GET['search'];
                        $sql = "SELECT * from  login_information , user_role , user_data WHERE login_information.user_id = user_data.user_id AND login_information.user_role_id = user_role.user_role_id AND firstname LIKE '%" . $search . "%' ORDER BY login_id DESC $limit ";
                        $strKeyword = $_GET['search'];// รับค่า search
                    }
...
<body>
    ...
    <div class="main">
        <div class="container">
            <div class="display-3 text-center">ข้อมูลการเข้าระบบ</div>
            <div class="col-xs-6">
                <a href="add_login.php" class="btn btn-success mb-3"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                        height="16" fill="currentColor" class="bi bi-person-plus-fill" viewBox="0 0 16 16">
                        <path d="M1 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z" />
                        <path fill-rule="evenodd"
                            d="M13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5z" />
                    </svg> เพิ่มผู้ใช้งาน</a>
            ...
            ?>
                    <tr>
                        ...
                        <td><?php if($row1["status_login"] == 'Active'){?>
                            <p class="active">ใช้งานได้</p>
                            <?php } elseif($row1["status_login"] == 'Inactive'){?>
                            <p class="inactive">ถูกระงับการใช้งานได้</p>
                            <?php } ?>
                        </td>
                        <td><a href="edit_login.php?update_id=<?php echo $row1["login_id"]; ?>" class="btn btn-warning">แก้ไข
                        </td>
                        <td><a href="delete_login.php?delete_id=<?php echo $row1["login_id"]; ?>" class="btn btn-danger "
                                onclick="return confirm('คุณต้องการลบข้อมูลหรือไม่')">ลบข้อมูล</td>
                        <td><a href="change_status_login.php?change_id=<?php echo $row1["login_id"]; ?>"
                                class="btn btn-info "
