<?php
    ...
    if(isset($_REQUEST['update_id'])){
        try{
            ...
            $select_stmt = $db->prepare("SELECT * FROM choose_a_teaching as report, login_information as user, subject, classroom as class, grade_level as grade
        ...
        if(empty($code_up)){
            ...
            try{
                if(!isset($errorMsg))
                    $update_stmt = $db->prepare("UPDATE choose_a_teaching SET grade_id = :grade_id, subject_id=:subject_id,class_id=:class_id,status_choose='Active' WHERE choose_id = :id");
                    $update_stmt->bindParam(':grade_id', $grade_level_user);
        ...
        <div class="form- text-center">
            ...
            <div class="col-sm-6">
                ...
                <?php foreach($result1 as $row1){
                    if($row1['status_subject'] == 'Active' && $row1['subject_id'] != $subject_id){?>
