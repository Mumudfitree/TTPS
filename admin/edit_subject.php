<?php
    ...
    if(isset($_REQUEST['update_id'])){
        try{
            ...
            $select_stmt = $db->prepare("SELECT * FROM subject, grade_level as grade WHERE subject.grade_id = grade.grade_id AND subject.subject_id = :id");
        ...
        if(empty($code_subject)){
            ...
            try{
                if(!isset($errorMsg))
                    $update_stmt = $db->prepare("UPDATE subject SET grade_id = :grade_level_user, code_subject = :code, name_subject = :name WHERE subject_id = :id");
                    ...
                    if($update_stmt->execute()){
                        $updateMeg = "อัพเดตขข้อมูลเสร็จสิ้น";
