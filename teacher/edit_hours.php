<?php
    ...
    if(isset($_REQUEST['update_id'])){
        try{
            ...
            $select_stmt = $db->prepare("SELECT * FROM prepare_to_teach as reportState, choose_a_teaching as report, subject, classroom as class, grade_level as grade
            WHERE reportState.id_prepare = :id AND report.subject_id = subject.subject_id AND report.class_id = class.class_id AND report.grade_id = grade.grade_id AND
            reportState.choose_id = report.choose_id ");
            $select_stmt->bindParam(':id', $id2);
            $select_stmt->execute();
        ...
        if(empty($subject_up)){
            ...
            try{
                if(!isset($errorMsg)){echo 'bf';
                    $update_stmt = $db->prepare("UPDATE prepare_to_teach SET  choose_id = '".$choose_id."', date_prepare ='".$date_prepare."'
