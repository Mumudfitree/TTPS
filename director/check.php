...
if (!isset($_SESSION['director_login'])) {//คำสั่งต้องloginก่อนถึงเข้าได้
...
$nquery=mysqli_query($conn,"SELECT * from  prepare_to_teach as pre,choose_a_teaching as c,subject as sub,classroom as class,grade_level as grade,login_information as login, user_data as user
WHERE user.user_id = login.user_id AND pre.choose_id = c.choose_id AND c.class_id = class.class_id AND c.subject_id = sub.subject_id AND c.grade_id = grade.grade_id  AND c.login_id = login.login_id ORDER BY id_prepare DESC $limit");
    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
            <?php
                ...
                ?>  
                        <tr>
                       <?php if($row['status_prepare_hours']=='Complete' &&$row['status_prepare'] == 'Active' ){ ?>
                        <td><?php echo $row['date_prepare']; ?></td>
                        <td><?php echo $row['firstname'].' '.$row['lastname']; ?></td>
                        <td><?php echo  $row['code_subject'].' '.$row['name_subject'].' ('.$row['name_classroom'].')'; ?></td>
                        <td><a target ="_blank" href="download_hours.php?download_id=<?php echo $row["id_prepare"]; ?>" class="btn btn-success">ดาวน์โหลด</a></td>
